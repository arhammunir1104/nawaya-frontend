...
const SurveyModal = ({ onClose }) => {
 ...
 useEffect(() => {
  ...
  if (isSubmitted) {
    return (
      <div className="fixed z-[9999] inset-0 h-[100vh] w-[100vw] flex items-center justify-center p-2">
        ...
        <div className="relative border-2 mt-[-10%] w-[100%] mr-[5%] ml-[5%] md:w-[60%] bg-white rounded-[40px] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-300">
          ...
          <p className="text-[#666666] text-center w-[100%] md:w-[60%] md:ml-[20%] text-[10px] md:text-xs leading-relaxed mb-2">
...
Limited to the first 500 members only. Once it’s gone, it’s gone.</p>
           ...
           <button onClick={onClose} type="button" className="btn-video cursor-pointer">
            <span className="btn-video__text text-[10px] md:text-xs xs:text-Paragraph6 2xl:text-Paragraph4">Close</span>
            </button>

            <NavLink to={"/exclusive"}>
             <button 
              className="px-10 py-3 rounded-full text-white cursor-pointer text-sm md:text-md shadow-xl transition-all hover:scale-105 active:scale-95"
              style={{
  ...
  // --- VIEW: SURVEY FORM ---
  return (
    <div className="fixed top-[10%] z-[9999] inset-0 h-[100vh] w-[100vw] flex items-center justify-center p-4 sm:p-6 md:p-10">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-300">
        ...
        <div className="w-full py-6 text-center shrink-0  md:mt-0" style={{ background: 'linear-gradient(90deg, #94BD1C 0%, #29C28C 100%)' }}>
          <div className='flex items-center justify-start p-5 absolute top-0 left-0 z-10'>
          <button onClick={onClose}>
            <IoMdArrowBack className="sm:text-3xl text-xl text-white hover:text-red-500 transition-all cursor-pointer" />
          </button>
        </div>
          <h2 className="text-white font-bold text-sm md:text-md sm:text-xl uppercase tracking-widest">Join The Waitlist</h2>
        </div>
        ...
        <div className="overflow-y-auto p-8 md:p-12 custom-scrollbar">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <p className='text-black text-[10px] md:text-xs text-left font-Urbanist'>We’re building <span className='font-bold'> Nawaya </span> together with our early community. Once we launch, we’ll reach out 
